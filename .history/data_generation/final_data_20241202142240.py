...
def split_weather_data_by_title():
    ...
    if os.path.exists(file_path):
        ...
        if not df.empty:
            ...
            # Rename and reorder columns as per the desired format
            final_df = final_df[['title', 'longitude', 'latitude', 'temperature', 'humidity',
                                 'high_cloud', 'low_cloud', 'mid_cloud', 'windspeed', 'windDirection', 'skycover']]

            # Merge Windspeed and WindDirection columns into one column
            final_df['windspeed_windDirection'] = final_df['windspeed'].astype(str) + ' ' + final_df['windDirection'].astype(str)

            # Drop the original 'windspeed' and 'windDirection' columns
            final_df = final_df.drop(columns=['windspeed', 'windDirection'])

            # Reorder columns to match the desired format
            final_df = final_df[['title', 'longitude', 'latitude', 'temperature', 'humidity', 
                                 'high_cloud', 'low_cloud', 'mid_cloud', 'windspeed_windDirection', 'skycover']]
