...
# Remove rows where 'Wind_direction' is NaN or missing
filtered_data = weather_data.dropna(subset=['Wind_direction'])
...
# Extract coordinates and Wind_direction values
weather_coords = np.array([(point.x, point.y) for point in weather_gdf.geometry])
pressure_values = filtered_data['Wind_direction'].values  # Now this contains valid Wind_direction values only
...
plt.savefig(os.path.join(current_dir, '..', 'assets', 'isogon_heatmap.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
crop_inches = 1
heatmap_path = os.path.join(current_dir, '..', 'assets', 'isogon_heatmap.png')
...
plt.savefig(os.path.join(current_dir, '..', 'assets', 'isogons.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
crop_inches = 1
isolines_path = os.path.join(current_dir, '..', 'assets', 'isogons.png')
