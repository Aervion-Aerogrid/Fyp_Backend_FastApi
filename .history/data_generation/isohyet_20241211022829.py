...
# Remove rows where 'Precipitation_rate' is NaN or missing
filtered_data = weather_data.dropna(subset=['Precipitation_rate'])
filtered_data = filtered_data[~np.isinf(filtered_data['Precipitation_rate'])]  # Remove infinite values
...
# Extract coordinates and Precipitation_rate values
weather_coords = np.array([(point.x, point.y) for point in weather_gdf.geometry])
rainfall_values = filtered_data['Precipitation_rate'].values  # Now this contains valid Precipitation_rate values only

# Check sparsity or variance of Sky_cover values
non_zero_count = np.count_nonzero(rainfall_values)
total_count = len(rainfall_values)
variance = np.var(rainfall_values)
...
if non_zero_count / total_count >= sparsity_threshold and variance >= variance_threshold:
    ...
    kriging_rainfall, ss = ok.execute('grid', x_points, y_points)
    kriging_rainfall, ss = ok.execute('grid', x_points, y_points)
    ...
    contour_filled = ax.contourf(x_grid, y_grid, kriging_rainfall, levels=15, cmap='coolwarm', alpha=0.7)
    ...
    plt.savefig(os.path.join(current_dir, '..', 'assets', 'isohyet_heatmap.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
    crop_inches = 1
    heatmap_path = os.path.join(current_dir, '..', 'assets', 'isohyet_heatmap.png')
    ...
    # Plot contour lines
    contour_lines = ax.contour(x_grid, y_grid, kriging_rainfall, levels=15, colors='black', linewidths=1.5)
    ...
    plt.savefig(os.path.join(current_dir, '..', 'assets', 'isohyets.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
    crop_inches = 1
    isolines_path = os.path.join(current_dir, '..', 'assets', 'isohyets.png')
