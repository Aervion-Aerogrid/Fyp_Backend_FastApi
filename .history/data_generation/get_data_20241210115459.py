...
def filter_and_process_weather_data(file_path):
    ...
    with open(file_path, 'r') as infile, open(filtered_file_path, 'w') as outfile:
        for line in infile:
            if line.strip()[:5].isdigit():  # Keep lines starting with a 5-digit number
                outfile.write(line)
    ...
    final_file_path = "../assets/synop_data.txt"
...
def process_synop_batch(synop_batch):
    ...
    for synop in synop_batch:
        ...
        if len(parts) >= 4:
            ...
            # Check if cleaned_humidity is not empty before converting to int
            if cleaned_humidity:
