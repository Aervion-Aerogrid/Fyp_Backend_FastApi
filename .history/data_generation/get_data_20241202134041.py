...
# List of coordinates with respective city names
coordinates = [
...
# Function to fetch and process the most recent weather data for each coordinate
def fetch_recent_weather_data():
    ...
    for city, (lat, lon) in coordinates:
        ...
        try:
            ...
            # Get the last data point for each variable
            hourly_data = {
                "city": city,
                "latitude": lat,
                "longitude": lon,
                "temperature_2m": hourly.Variables(0).ValuesAsNumpy()[-1],
                "relative_humidity_2m": hourly.Variables(1).ValuesAsNumpy()[-1],
                "dew_point_2m": hourly.Variables(2).ValuesAsNumpy()[-1],
                "precipitation": hourly.Variables(3).ValuesAsNumpy()[-1],
                "surface_pressure": hourly.Variables(4).ValuesAsNumpy()[-1],
                "cloud_cover": hourly.Variables(5).ValuesAsNumpy()[-1],
                "wind_speed_10m": hourly.Variables(6).ValuesAsNumpy()[-1],
                "wind_direction_10m": hourly.Variables(7).ValuesAsNumpy()[-1],
            }
            
            # Append the most recent data point to the list
            all_data.append(hourly_data)

        except Exception as e:
            print(f"Error fetching data for {city} ({lat}, {lon}): {e}")

    if all_data:
        # Create a DataFrame for the most recent data
        recent_data_df = pd.DataFrame(all_data)
        ...
        file_path = os.path.join(assets_dir, 'recent_weather_data.csv')
        recent_data_df.to_csv(file_path, index=False)

        print(f"Most recent data saved to {file_path}")
    else:
