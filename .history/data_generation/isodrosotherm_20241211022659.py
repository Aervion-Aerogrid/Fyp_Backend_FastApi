...
# Remove rows where 'Dew_point' is NaN or missing
filtered_data = weather_data.dropna(subset=['Dew_point'])
...
# Extract coordinates and Dew_point values
weather_coords = np.array([(point.x, point.y) for point in weather_gdf.geometry])
dpoint_values = filtered_data['Dew_point'].values  # Now this contains valid Dew_point values only
...
kriging_dpoint, ss = ok.execute('grid', x_points, y_points)

# Plotting Kriging Heatmap
fig, ax = plt.subplots(figsize=(12, 12))

# Plot filled contours for heatmap
contour_filled = ax.contourf(x_grid, y_grid, kriging_dpoint, levels=15, cmap='coolwarm', alpha=0.7)
...
plt.savefig(os.path.join(current_dir, '..', 'assets', 'isodrosotherm_heatmap.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
crop_inches = 1
heatmap_path = os.path.join(current_dir, '..', 'assets', 'isodrosotherm_heatmap.png')
...
# Plot contour lines
contour_lines = ax.contour(x_grid, y_grid, kriging_dpoint, levels=15, colors='black', linewidths=1.5)
...
plt.savefig(os.path.join(current_dir, '..', 'assets', 'isodrosotherms.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
crop_inches = 1
isolines_path = os.path.join(current_dir, '..', 'assets', 'isodrosotherms.png')
