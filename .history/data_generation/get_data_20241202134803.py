    ...
    for city, (lat, lon) in coordinates:
        ...
        try:
            ...
            for response in responses:
                ...
                # Extract the data and keep only the latest timestamp
                most_recent_time = timestamps.max()
                latest_index = timestamps[timestamps == most_recent_time].index[0]

                # Extract variables for the most recent time
                latest_data = {
                    "city": city,
                    "latitude": lat,
                    "longitude": lon,
                    "date": most_recent_time,
                    "temperature_2m": hourly.Variables(0).ValuesAsNumpy()[latest_index],
                    "relative_humidity_2m": hourly.Variables(1).ValuesAsNumpy()[latest_index],
                    "dew_point_2m": hourly.Variables(2).ValuesAsNumpy()[latest_index],
                    "precipitation": hourly.Variables(3).ValuesAsNumpy()[latest_index],
                    "surface_pressure": hourly.Variables(4).ValuesAsNumpy()[latest_index],
                    "cloud_cover": hourly.Variables(5).ValuesAsNumpy()[latest_index],
                    "wind_speed_10m": hourly.Variables(6).ValuesAsNumpy()[latest_index],
                    "wind_direction_10m": hourly.Variables(7).ValuesAsNumpy()[latest_index],
                }
