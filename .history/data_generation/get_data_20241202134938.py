...
def fetch_weather_data():
    ...
    if all_data:
        ...
        file_path = os.path.join(assets_dir, 'weather_data_by_city.xlsx')
        with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
            for city, df in all_data.items():
                df.to_excel(writer, sheet_name=city[:30], index=False)  # Sheet names are limited to 30 characters
