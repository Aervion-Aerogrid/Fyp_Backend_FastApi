...
# Remove rows where 'Humidity' is NaN or missing
filtered_data = weather_data.dropna(subset=['Humidity'])
...
# Extract coordinates and Humidity values
weather_coords = np.array([(point.x, point.y) for point in weather_gdf.geometry])
pressure_values = filtered_data['Humidity'].values  # Now this contains valid Humidity values only
...
plt.savefig(os.path.join(current_dir, '..', 'assets', 'isohume_heatmap.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
crop_inches = 1
heatmap_path = os.path.join(current_dir, '..', 'assets', 'isohume_heatmap.png')
...
plt.savefig(os.path.join(current_dir, '..', 'assets', 'isohumes.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
crop_inches = 1
isolines_path = os.path.join(current_dir, '..', 'assets', 'isohumes.png')
