...
# Remove rows where 'Wind_speed' is NaN or missing
filtered_data = weather_data.dropna(subset=['Wind_speed'])
...
# Extract coordinates and Wind_speed values
weather_coords = np.array([(point.x, point.y) for point in weather_gdf.geometry])
pressure_values = filtered_data['Wind_speed'].values  # Now this contains valid Humidity values only
...
plt.savefig(os.path.join(current_dir, '..', 'assets', 'isotach_heatmap.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
crop_inches = 1
heatmap_path = os.path.join(current_dir, '..', 'assets', 'isotach_heatmap.png')
...
plt.savefig(os.path.join(current_dir, '..', 'assets', 'isotachs.png'), dpi=300, bbox_inches='tight', transparent=True, pad_inches=0)
crop_inches = 1
isolines_path = os.path.join(current_dir, '..', 'assets', 'isotachs.png')
