...
def process_synop_row(token):
    ...
    if 0 <= wind_direction <= 360:
        rounded_wind_direction = round(wind_direction / 30) * 30
    else:
        rounded_wind_direction = None
        
    if rounded_wind_direction == 360:
        rounded_wind_direction = 0
    
    
    
    wind_speed = safe_int(token[2][3:])  # Wind speed (knots)
    
    # Check the range and round if valid
    if 0 <= wind_speed <= 100:
        rounded_wind_speed = round(wind_speed / 5) * 5
    else:
        rounded_wind_speed = None  # Nullify if out of range
    ...
    station_data = {
        ...
        "Wind_direction":rounded_wind_direction,
        "Wind_speed": rounded_wind_speed,
...
# Define file paths
input_file_path = os.path.join(assets_dir, 'synop_data.txt')
output_file_path = os.path.join(assets_dir, 'weather_data.csv')

# Run the function to decode and save
